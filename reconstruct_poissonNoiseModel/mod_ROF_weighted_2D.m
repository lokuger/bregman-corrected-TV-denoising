function [u,q] = mod_ROF_weighted_2D(g,h,mu)
...
%
% Input:    g   -   noisy image (passed as a M*N x 1 vector)
%           h   -   vector of weights for the l2 norm (M*N x 1)
%           mu  -   regularization parameter
%           M,N -   original size of the image (passed as a reshaped M*N x 1)
...
while ~stopcrit
    ...
    stopcrit = max(max(abs(px-poldx),[],'all'),max(abs(py-poldy),[],'all')) < 1e-2;
end
q = div(px,py);
u = g - mu * h.* q;
